...
export class TidewaysAPIError extends Error {
    ...
    statusCode;
    retryAfter;
    constructor(message, category, statusCode, retryAfter) {
        super(message);
...
export class ErrorHandler {
    static getJsonRpcErrorCode(error) {
    ...
    static handleApiError(error) {
        ...
        if (error.response?.status === 429) {
            ...
            return new TidewaysAPIError('Rate limit exceeded. Please try again later.', 'rate_limit', 429, retryAfter);
        }
        if (error.response?.status === 401) {
            return new TidewaysAPIError('Authentication failed. Please check your API token.', 'auth', 401);
        }
        if (error.response?.status === 403) {
            return new TidewaysAPIError('Access denied. Your token may not have the required scopes for this operation.', 'auth', 403);
        }
        if (error.response?.status === 404) {
            return new TidewaysAPIError('Resource not found. Please check your organization and project settings.', 'api', 404);
        }
        if (error.response?.status >= 500) {
            return new TidewaysAPIError('Tideways API server error. Please try again later.', 'api', error.response.status);
        }
        if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
            return new TidewaysAPIError('Network error: Unable to connect to Tideways API. Please check your internet connection.', 'network');
        }
        if (error.code === 'ECONNABORTED') {
            return new TidewaysAPIError('Request timeout: Tideways API took too long to respond.', 'network');
        }
    ...
    static handlePartialFailure(errors, partialData) {
